   ...
   for root_domain_name, folder in domain_to_folder.items():
       ...
       if os.path.exists(hostnames_file_path) and os.path.getsize(hostnames_file_path) > 0:
           with open(hostnames_file_path, 'r') as file:
               ...
               for hostname in hostnames:
                   if hostname not in blacklist_sub_domains and extract_domain(hostname, root_domain_name) == root_domain_name and hostname not in existing_sub_domains.keys():
           ...
           with open(servers_file_path, 'r') as file:
               ...
               for server in servers:
                   ...
                   if url.scheme and url.netloc and server not in blacklist_urls:
                       hostname = url.netloc
                       if hostname in existing_sub_domains.keys() or extract_domain(hostname, root_domain_name) == root_domain_name:
                           sub_domain_id_str = existing_sub_domains.get(hostname, None)
                           existing_site = db.site.find_one({"url": server, "root_domain_id": root_domain_id_str})
